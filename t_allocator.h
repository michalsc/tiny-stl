...
public:
    ...
    allocator() : construct_count(0), deconstruct_count(0), allocate_count(0), deallocate_count(0) {};
    allocator(const allocator&) : allocator() {};
    ~allocator() { printf("Constructed: %d, Deconstructed: %d\nAllocated: %d, Deallocated: %d\n", construct_count, deconstruct_count, allocate_count, deallocate_count); }
    pointer address(reference x) { return &x; }
    const_pointer address(const_reference x) { return &x; }
    pointer allocate(size_type n) { pointer p = (pointer)AllocMem(n * sizeof(value_type), MEMF_CLEAR); printf("t_std::allocate(%d)=%p\n", (int)n, (void*)p); allocate_count++; return p; }
    void deallocate(pointer p, size_type n) { printf("t_std::deallocate(%p, %d)\n", (void*)p, (int)n); if (n>0 && p!=nullptr) FreeMem((void*)p, n * sizeof(T)); deallocate_count++; }
